...
test('order HTTP boundary requires native session, rights, origin and server tax mapping',async()=>{
 ...
 const queue={enqueue:async c=>{queued++;assert.equal(c.actorId,user.id);assert.equal(c.operation,'order');assert.equal(c.payload.taxId,4);return {requestId:c.requestId,state:'queued',result:null};},status:async(id,actor)=>{assert.equal(actor,user.id);return null;}};
 const origin='https://ma2f.example.invalid';
 const env={MA2F_AUTH_ORIGIN:origin,MA2F_LOCAL_AUTH_ENABLED:'true',MA2F_RUNTIME_ENABLED:'true',MA2F_ODOO_ORDERS_ENABLED:'true',MA2F_ODOO_SALE_TAX_ID:'4'};
 const app=express();app.use('/orders',orderRouter(env,{auth,queue}));app.use('/off',orderRouter({}));
 const server=app.listen(0,'127.0.0.1');await new Promise(r=>server.once('listening',r));
 const base=`http://127.0.0.1:${server.address().port}`;
 const body={requestId:randomUUID(),customerId:7,packs:7,unitPriceIncludedFCFA:600};
 ...
 try{
  ...
  env.MA2F_ODOO_SALE_TAX_ID='';assert.equal((await post()).status,503);env.MA2F_ODOO_SALE_TAX_ID='4';
